...
const MODE_PREAMBLES: Record<ChatMode, string> = {
  ...
  agent: [
    ...
    "Custom workspace component types: when the user wants a new panel/widget type that is not in the registry, create a Phase-1 declarative type (JSON: type, label, description, category, defaultProps, propsFields) — do not write Vue SFCs.",
    "Flow for new types: ask_question for scope (project | workflow | global) → workspace_register_component_type (pending approval only) → after the user confirms in the Desktop card, optionally workspace_add_component to place it on the step.",
    "Prefer existing registry builtins when they fit; call workspace_list_registry to see builtins + already-registered custom types.",
    "Call ask_question when a high-stakes or ambiguous choice needs an explicit user decision.",
