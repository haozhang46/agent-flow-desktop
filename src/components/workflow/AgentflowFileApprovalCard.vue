...
<template>
  ...
  >
    <p class="text-xs font-medium text-amber-900 uppercase tracking-wide">
      .agentflow file change pending approval
    </p>
    <p class="text-xs text-gray-700">{{ summary }}</p>
    <p class="text-[10px] font-mono text-gray-500 break-all">{{ path }}</p>
    <template v-if="!compact">
      <div class="grid grid-cols-1 gap-2 md:grid-cols-2">
        <div>
          <p class="text-[10px] font-medium text-gray-500 mb-1">Before</p>
          <pre class="text-[10px] whitespace-pre-wrap text-gray-700 max-h-32 overflow-y-auto bg-white border border-gray-200 rounded p-2">{{ preview(before) }}</pre>
        </div>
        <div>
          <p class="text-[10px] font-medium text-gray-500 mb-1">After</p>
          <pre class="text-[10px] whitespace-pre-wrap text-gray-700 max-h-32 overflow-y-auto bg-white border border-gray-200 rounded p-2">{{ preview(after) }}</pre>
        </div>
      </div>
    </template>
    <details v-else class="text-[10px] text-gray-600">
      ...
      <div class="grid grid-cols-1 gap-2 mt-2 md:grid-cols-2">
        <pre class="whitespace-pre-wrap text-gray-700 max-h-24 overflow-y-auto bg-white border border-gray-200 rounded p-2">{{ preview(before) }}</pre>
        <pre class="whitespace-pre-wrap text-gray-700 max-h-24 overflow-y-auto bg-white border border-gray-200 rounded p-2">{{ preview(after) }}</pre>
      </div>
    </details>
